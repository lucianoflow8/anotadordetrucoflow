...
export const Game = () => {
  ...
  const setCurrentPoints = (newPointsObject, isWe, value) => {
    ...
    if (value === 30) {
      ...
      setTimeout(() => {
        setModalMessage({
          title: `${isWe ? 'Nosotros los Garchamos JAJA' : 'Ellos nos cogieron'} por el olli ;(`,
          subtitle: '¿Cojemos de nuevo? 8==D🙂'
        })
        setShowModal(true)
      }, 300)
    }
  ...
  return (
    <section className="flex flex-col">
      <div className="flex flex-row text-white gap-2 items-stretch">
        <CounterBoard
          title="Nosotros"
          onChangePoints={handleWeOnChange}
          points={points.we}
          showMusicButton={false} // Sin botón música real
          showMusicButtonPlaceholder={true} // Botón invisible más grande
        />
        <CounterBoard
          title="Ellos"
          onChangePoints={handleTheyOnChange}
          points={points.they}
          showMusicButton={true} // Con botón música real
        />
      </div>
